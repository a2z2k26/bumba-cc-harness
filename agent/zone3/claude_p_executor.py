...
def build_claude_p_argv(*, claude_binary: str, model: str | None = None) -> list[str]:
    """Build the ``claude -p`` argv. Pure — no I/O, no secrets.
...
async def run_claude_p_specialist(
    *,
    ...
    """
    argv = build_claude_p_argv(claude_binary=claude_binary, model=model)
