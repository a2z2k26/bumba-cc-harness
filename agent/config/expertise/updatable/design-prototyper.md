...
- bumba-desktop is the active desktop project (per operator memory: 6 modal tabs — Dashboard, Activity, Agents, Projects, Dreamcatcher, Settings). Prototypes for those tabs go to `bumba-desktop/prototypes/<tab>/` per that repo's structure, not into bumba-cc-harness.
