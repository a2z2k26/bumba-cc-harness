...
class BridgeConfig:
    ...
    backends_main: str = "claude"
    backends_chiefs_default: str = "claude"
    backends_specialists_default: str = "claude"
    ...
    # the bumba-cc-harness repository by default. Operators can override this path,
...
def _validate(config: BridgeConfig) -> None:
    ...
    # Claude OAuth token. The bridge cannot invoke ``claude -p`` without it,
    # and a silent empty default produces an opaque subprocess error on the
    # first API call. Sibling check to the boot-time validator
    ...
    if not getattr(config, REQUIRED_BOOT_SECRET_KEYS[0]):
        raise ConfigError(
