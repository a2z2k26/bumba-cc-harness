...
from .config import BridgeConfig, load_config  # noqa: F401
from .wiring import (
    WiringEntry,
    WiringReport,
...
def _validate_claude_oauth_required(config: object) -> None:
    """Refuse to boot when ``claude_oauth_token`` is missing from .secrets.
    ...
    full ``BridgeApp``. No I/O — reads only ``config.claude_oauth_token``.

    Raises:
        RuntimeError: when ``claude_oauth_token`` is empty. The message names
            the missing field, the rotation companion fields, the canonical
            secrets path, the mode discipline, and the restart action.
    """
