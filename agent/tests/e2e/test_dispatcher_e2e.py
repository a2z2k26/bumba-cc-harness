...
async def test_dispatcher_resume_session_threads_through_runner(
    ...
    """SubagentExecutor builds session_id="subagent-<wo_id_prefix>" and the
    real ClaudeRunner threads it through to ``--resume``. The shim returns
    that session_id via system:init, so we can assert the round trip."""
    ...
    expected_prefix = f"subagent-{wo.id[:8]}"
    # Shim returns args.resume as session_id when --resume is passed
    assert cr.session_id == expected_prefix, (
        f"Expected session_id={expected_prefix!r}, got {cr.session_id!r}. "
        "ClaudeRunner did not thread session_id through --resume."
    )
