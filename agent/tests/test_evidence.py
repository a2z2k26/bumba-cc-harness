...
class TestEvidenceConfig:
    def test_load_parses_repos(self, tmp_path):
        ...
        toml_path = make_evidence_toml(
            [{"name": "your-org/bumba-cc-harness", "commands": [["pytest", "tests/"]]}],
            tmp_path,
        )
        config = EvidenceConfig.load(toml_path)
        assert len(config.specs) == 1
        assert config.specs[0].name == "your-org/bumba-cc-harness"

    def test_load_parses_commands(self, tmp_path):
        toml_path = make_evidence_toml(
            [
                {
                    "name": "your-org/bumba-cc-harness",
        ...
        config = EvidenceConfig.load(toml_path)
        cmds = config.commands_for_repo("your-org/bumba-cc-harness")
        assert len(cmds) == 2
    ...
    def test_commands_for_unknown_repo_returns_empty(self, tmp_path):
        ...
        toml_path = make_evidence_toml(
            [{"name": "your-org/bumba-cc-harness", "commands": [["pytest"]]}],
            tmp_path,
        )
        config = EvidenceConfig.load(toml_path)
    ...
    def test_timeout_defaults_to_300(self, tmp_path):
        ...
        config = EvidenceConfig.load(toml_path)
        assert config.timeout_for_repo("your-org/bumba-cc-harness") == 300

    def test_timeout_custom_value_loaded(self, tmp_path):
        toml_path = make_evidence_toml(
            [{"name": "your-org/bumba-cc-harness", "commands": [["pytest"]], "timeout_seconds": 60}],
            tmp_path,
        )
        config = EvidenceConfig.load(toml_path)
        assert config.timeout_for_repo("your-org/bumba-cc-harness") == 60
    ...
    def test_real_evidence_toml_is_parseable(self):
        ...
        # bumba-cc-harness must be in there
        cmds = config.commands_for_repo("your-org/bumba-cc-harness")
        assert len(cmds) >= 1
...
class TestCreatePrWithEvidence:
    ...
    def _make_config(self, tmp_path: Path, commands=None) -> EvidenceConfig:
        ...
        toml_path = make_evidence_toml(
            [{"name": "your-org/bumba-cc-harness", "commands": cmds}],
            tmp_path,
        )
    ...
    def test_happy_path_returns_pr_url(self, tmp_path):
        ...
        gh_runner = make_gh_runner(stdout="https://github.com/your-org/bumba-cc-harness/pull/100")

        url = asyncio.run(
            create_pr_with_evidence(
                title="fix: the bug",
                body="## Summary\nFixed it.",
                repo="your-org/bumba-cc-harness",
                config=config,
                cwd=tmp_path,
                evidence_root=tmp_path / ".harness" / "evidence",
        ...
        assert url == "https://github.com/your-org/bumba-cc-harness/pull/100"
    ...
    def test_empty_commands_still_creates_pr(self, tmp_path):
        ...
        toml_path = make_evidence_toml(
            [{"name": "your-org/bumba-cc-harness", "commands": []}],
            tmp_path,
        )
        config = EvidenceConfig.load(toml_path)
