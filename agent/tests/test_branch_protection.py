...
def _strict_ok_mock() -> object:
    ...
    m = AsyncMock(
        return_value=ProtectionResult(
            ...
            ok=True,
            degraded=False,
            reason="your-org/bumba-cc-harness/main is protected per the standard ruleset.",
        )
    )
    return m


def _unprotected_mock(repo: str = "your-org/bumba-cc-harness") -> object:
...
def test_check_branch_protection_passes_strict_ok():
    ...
    with patch("bridge.branch_protection._run_gh", _mock_run_gh(
        ...
        result = check_branch_protection("your-org/bumba-cc-harness", posture="warn")
...
def test_check_branch_protection_block_posture_rejects_unprotected():
    ...
    with patch("bridge.branch_protection._run_gh", _mock_run_gh(
        ...
        result = check_branch_protection("your-org/bumba-cc-harness", posture="block")

    assert result.passed is False
    assert result.category == "branch_protection"
    assert "FAILED" in result.reason
    assert "your-org/bumba-cc-harness" in result.reason
...
def test_check_branch_protection_emits_event_bus_event_on_degraded():
    ...
    with patch.object(EventBus, "get_instance", return_value=_FakeInstance()):
        with patch("bridge.branch_protection._run_gh", _mock_run_gh(
            ...
            check_branch_protection("your-org/bumba-cc-harness", posture="warn")

    assert len(published_events) == 1
    evt = published_events[0]
    assert evt["event_type"] == "security.branch_protection.failed"
    assert evt["payload"]["repo"] == "your-org/bumba-cc-harness"
