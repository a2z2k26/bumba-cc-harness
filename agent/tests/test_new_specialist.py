...
from scripts._scaffolding_templates import expertise_for, worker_prompt_for
...
def _write_team_yaml(tmp_path: Path, team: str) -> Path:
    ...
    p.write_text(MINIMAL_TEAM_YAML.format(team=team), encoding="utf-8")
    return p
...
def test_new_specialist_success(fake_repo: Path):
    ...
    assert worker["model"].startswith("openrouter:")
    assert worker["adapter"] == "openrouter"
