...
def _assert_openrouter_models_declare_openrouter_adapter(yaml_content: str) -> None:
    ...
    offenders = [
        ...
        if str(member.get("model", "")).startswith("openrouter:")
        and member.get("adapter") != "openrouter"
    ]
    assert offenders == []
...
class TestSingleAgentBundle:
    ...
    def test_openrouter_chief_declares_openrouter_adapter(self):
        _assert_openrouter_models_declare_openrouter_adapter(
...
class TestAgentTeamBundle:
    ...
    def test_openrouter_members_declare_openrouter_adapter(self):
        _assert_openrouter_models_declare_openrouter_adapter(
            _extract_team_yaml(self.result)
        )
