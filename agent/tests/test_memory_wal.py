...
async def test_drain_skipped_while_consolidation_lock_held(
    ...
    # Simulate another live process holding the lock by writing a PID
    # that's NOT our own but is currently alive (use the parent PID — for
    # the test runner, that's the test runner itself or a launcher; either
    # way it's alive).
    other_pid = os.getppid()
    if other_pid in (0, os.getpid()):
        pytest.skip("Cannot simulate other live PID in this environment")
